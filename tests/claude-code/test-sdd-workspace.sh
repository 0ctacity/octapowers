...
main() {
    ...
    if [[ "$dir" == "$repo/.octapowers/sdd" ]]; then
        pass "prints <repo-root>/.octapowers/sdd"
    else
        fail "prints <repo-root>/.octapowers/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.octapowers/sdd/.gitignore" && "$(cat "$repo/.octapowers/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.octapowers/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.octapowers/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    rp_out="$(cd "$repo" && "$SDD_SCRIPTS/review-package" HEAD)"
    rp_path="$(printf '%s\n' "$rp_out" | sed -n 's/^wrote \(.*\): [0-9].*$/\1/p')"
    case "$rp_path" in
        "$repo/.octapowers/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
            fail "review-package writes its diff under the workspace"
            echo "    got: $rp_path"
            ;;
    esac
    if grep -Fq 'working change' "$rp_path"; then
        pass "review-package includes uncommitted delegated work"
    else
        fail "review-package includes uncommitted delegated work"
    fi
    ...
    if [[ "$wt_dir" == "$wt_root/.octapowers/sdd" && "$wt_dir" != "$dir" ]]; then
